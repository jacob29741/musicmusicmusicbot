...
token = "ODE3OTk5NjUzNTAwNzQ3ODA2.YERriw.Eith3RpeX9R8CNoIz8SGycQcYgQ"
prf = "ㅈ"
...
async def on_message(message):
    ...
    if message.content.startswith(f"{prf}음검"):
        url = message.content[4:]
        print(url)
        if "http" in url:
            return await message.channel.send(
    ...
    if message.content == f"{prf}스킵":
        ...
        try:
            if message.guild.voice_client.channel.id == message.author.voice.channel.id:
                ...
                return await message.channel.send("자곱봇과 같은 음성채널에 있어야합니다!")
        except AttributeError:
            return await message.channel.send(
                embed=discord.Embed(
...
kbtoken = "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjgxNzk5OTY1MzUwMDc0NzgwNiIsImlhdCI6MTYyNDQyMTkwMn0.Sm8U1EuxvKEbcx8OSks1CGlSUTNcEO63Y4LPpsO79_qE38Ct5RZgjgCtWeFTVqT03RONGQRUr4pYv36-d2sbCimyRcpO1aZvwrm5Xxq03Hvn0rFXZqZ2Q6npvuH08ZRRM9YSjMWS9o4vJhtIqbjUSH5WXG79nmFugAXiXohd7gY"
serverCount = 73 # 건들지마셈
response = post(f'{BASEURL}/bots/servers', headers={"token":kbtoken, "Content-Type": "application/json"}, json={"servers": serverCount})
print(response.json())
"""
